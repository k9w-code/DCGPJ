...
class GameEngine {
  ...
  endTurn(playerId) {
    ...
    const currentPlayerId = gs.playerOrder[gs.currentPlayerIndex];
    if (playerId !== currentPlayerId) return { error: '自分のターンではありません' };
    // ターン終了はmainフェーズ以外では受け付けない（targeting/discarding/shield_break_anim中は無効）
    if (gs.phase !== 'main') return { error: `現在のフェーズ(${gs.phase})ではターン終了できません` };
  ...
  // 遺言などで連鎖死亡が起きる可能性があるため、死亡ユニットがなくなるまでループ
  cleanupDeadUnits() {
    let anyDied = true;
    let safetyCount = 0;
    const MAX_CLEANUP_LOOPS = 20; // 無限ループ防止
    while (anyDied && safetyCount < MAX_CLEANUP_LOOPS) {
      anyDied = false;
      safetyCount++;
    ...
    if (safetyCount >= MAX_CLEANUP_LOOPS) {
      console.warn('[GameEngine] cleanupDeadUnits: MAX_CLEANUP_LOOPS reached. Possible infinite death loop.');
    }
  }
