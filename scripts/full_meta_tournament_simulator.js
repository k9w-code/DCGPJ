// scripts/full_meta_tournament_simulator.js
// 修正版カードプール（BL024 2コスト）における単色＋有力2色全12アーキタイプのメタトーナメント
'use strict';
...
const REPORT_FILE = path.join(__dirname, '../data/full_meta_tournament_result.json');
...
const DECK_RECIPES = {
  ...
  d10_blue_resonance: {
    name: '【青単】共鳴呪文コンボ',
    type: '単色',
    cards: ['BL001','BL001','BL001','BL003','BL003','BL003','BL026','BL026','BL026','BL027','BL027','BL027','BL009','BL009','BL009','BL006','BL006','BL006','BL024','BL024','BL024','BL028','BL028','BL028','BL010','BL010','BL010','BL013','BL013','BL013','BL014','BL014','BL014','BL012','BL012','BL012','BL029','BL029','BL029','BL030']
  },
...
async function runFullTournament() {
  ...
  console.log(`\n🏆 全アーキタイプ 最新環境メタトーナメント (${deckKeys.length}デッキ)`);

  const MATCHES_PER_PAIR = 50; // 各対戦ペア50試合
  const results = {};
  deckKeys.forEach(k => {
  ...
  for (let i = 0; i < deckKeys.length; i++) {
    ...
    for (let j = i; j < deckKeys.length; j++) {
      ...
      for (let m = 0; m < MATCHES_PER_PAIR; m++) {
        ...
        totalTurns += engine.gameState.turnCount || 0;
